	...
	function render( $attrs, $content = null, $render_slug ) {
		...
		if ( have_posts() ) {
			...
			while ( have_posts() ) {
				...
				$no_thumb_class = '' === $thumb || ! $multi_view->has_value( 'show_thumbnail', 'on' ) ? ' et_pb_no_thumb' : '';
			...
			<?php
				...
				if ( ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) || post_password_required( $post ) ) {
					...
					elseif ( 'gallery' === $post_format ) :
						et_pb_gallery_images( 'slider' );
					elseif ( '' !== $thumb && $multi_view->has_value( 'show_thumbnail', 'on' ) ) :
						if ( 'on' !== $fullwidth ) {
			...
			<?php if ( 'off' === $fullwidth || ! in_array( $post_format, array( 'link', 'audio', 'quote' ) ) || post_password_required( $post ) ) { ?>
				...
				<?php
					...
					$remove_metas = array();
					$meta_datas   = array(
						'show_author'     => et_get_safe_localization( sprintf( __( 'by %s', 'et_builder' ), '<span class="author vcard">' .  et_pb_get_the_author_posts_link() . '</span>' ) ),
						'show_date'       => et_get_safe_localization( sprintf( __( '%s', 'et_builder' ), '<span class="published">' . esc_html( get_the_date( $meta_date ) ) . '</span>' ) ),
						'show_categories' => get_the_category_list(', '),
					...
					foreach ( $modes as $mode ) {
						if ( ! isset( $remove_metas[ $mode ] ) ) {
							$remove_metas[ $mode ] = array();
						}

						foreach ( array_keys( $meta_datas ) as $meta_data_key ) {
							if ( $multi_view->has_value( $meta_data_key, 'on', $mode, true ) ) {
								continue;
							}

							$remove_metas[ $mode ][ $meta_data_key ] = $meta_data_key;
						}

						$remove_metas[ $mode ] = implode( ',', $remove_metas[ $mode ] );
					}

					$remove_metas = $multi_view->distinct_values( $remove_metas );

					$meta_outputs = array();

					foreach ( $remove_metas as $mode => $remove_meta ) {
						$meta_outputs[ $mode ] = $meta_datas;

						if (! $remove_meta ) {
							continue;
						}

						foreach ( explode( ',', $remove_meta ) as $remove_meta_key ) {
							unset( $meta_outputs[ $mode ][ $remove_meta_key ] );
						}
					}

					if ( $meta_outputs ) {
						foreach ( $meta_outputs as $mode => $meta_contents ) {
							$meta_outputs[ $mode ] = implode( ' | ', $meta_contents );
						}

						$multi_view->set_custom_prop( 'meta_outputs', $meta_outputs );

						$multi_view->render_element( array(
							'tag'     => 'p',
							'content' => '{{meta_outputs}}',
							'attrs'   => array(
								'class' => 'post-meta',
							)
						), true );
					...
					if ( et_pb_is_pagebuilder_used( get_the_ID() ) ) {
						$more = 1; // phpcs:ignore WordPress.Variables.GlobalVariables.OverrideProhibited
						$multi_view_content = et_core_intentionally_unescaped( apply_filters( 'the_content', $post_content ), 'html' );
					} else {
						$more = null; // phpcs:ignore WordPress.Variables.GlobalVariables.OverrideProhibited
						$multi_view_content = et_core_intentionally_unescaped( apply_filters( 'the_content', et_delete_post_first_video( get_the_content( esc_html__( 'read more...', 'et_builder' ) ) ) ), 'html' );
					}

					$multi_view_excerpt = '';
					
					if ( has_excerpt() && 'off' !== $use_manual_excerpt ) {
						$multi_view_excerpt = get_the_excerpt();
					} else {
						$multi_view_excerpt = et_core_intentionally_unescaped( wpautop( et_delete_post_first_video( strip_shortcodes( truncate_post( $excerpt_length, false, '', true ) ) ) ), 'html' );
					}

					$multi_view->set_custom_prop( 'content', $multi_view->get_values( 'show_content' ) );

					$multi_view->set_conditional_value( 'content', $multi_view_content, array(
						'show_content' => 'on',
					) );

					$multi_view->set_conditional_value( 'content', $multi_view_excerpt, array(
						'show_content' => 'off',
					) );

					$multi_view->set_conditional_value( 'show_excerpt', 'on', array(
						'show_content' => 'on',
					) );
